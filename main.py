...
def parar_pop_1000(algoritmo):
    if len(algoritmo.populacoes) == 100:
        return True
    return False
...
def funcao_parada(algoritmo):
    ...
    for indv in algoritmo.populacoes[algoritmo.get_geracao_atual()]:
        if algoritmo.populacoes[algoritmo.get_geracao_atual()][indv]['rate'] == 0:
            count+=1
        if count >= 1:
            return True
    return False

funcoes_parada = [parar_pop_1000, funcao_parada]
cursos = ['lcc','si']
a = algoritmo('csv/salas.csv', 'csv/horarios.csv', 'csv/cadeiras.csv', 2, 500, requisitos, funcoes_parada, 1,cursos)
a.main()
